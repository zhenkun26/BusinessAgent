# Hello，小A——企业知识工作流 Agent — 产品文档

> 版本:v1.3
> 日期:2026-07-27(内容基线)/ 2026-08-04(文档迁移校验)
> 对应项目版本:v1.0.0(2026-08-04 生产级基线,见根目录 CHANGELOG.md 与 Git tag v1.0.0)
> 阶段:W2-W9 全部完成 + 整体联调(17/17 通过)+ P1/P2 迭代完成 + 生产对抗性审查修复完成
...
| 分支 | 条件 | 行为 | LLM |
|------|------|------|-----|
| 闲聊 | intent=chitchat | 小A 人格话术九分支(问候含用户称呼/自我介绍/心情状态/能力询问/日常寒暄/情绪共情/感谢/再见 + 未命中时 **lite LLM 即兴生成**,LLM 失败回默认模板),confidence=0.9 | 仅未命中分支时(lite 本地) |
...
| aggregator 闲聊 | 命中分支:不调 LLM(小A 人格话术模板);未命中:本地 qwen3.5:4b 即兴生成 | 模板零成本;生成走本地仍零云端成本(2026-07-28 起) |
...
- [ ] **UAT + 安全测试**:真实用户验收 + Prompt 注入攻击面实测
...
- [ ] **needs_replan 语义复查**:知识问答正常回答但响应偶发 `needs_replan=true`
...
- **已知瓶颈**:知识问答端到端 40-110s,主要耗在 LLM 串行链路(检索→生成→自评)与模型冷加载;并发承载未压测(P3,vegeta/k6)
...
| 缺口 | — | 无在线质量回流(feedback 刚建闭环,未接评测);prompt A/B 效果对比靠人工看审计;无压测基线 |
