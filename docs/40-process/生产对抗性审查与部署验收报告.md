...
```bash
kubectl -n hello-zhiduoxing create secret generic agent-secrets \
...
kubectl -n hello-zhiduoxing rollout status deployment/agent-api
...
```bash
kubectl -n hello-zhiduoxing rollout undo deployment/agent-api
kubectl -n hello-zhiduoxing rollout history deployment/agent-api
kubectl -n hello-zhiduoxing rollout undo deployment/agent-api --to-revision=<N>
kubectl -n hello-zhiduoxing rollout status deployment/agent-api
```
...
```bash
kubectl -n hello-zhiduoxing get configmap agent-config -o yaml > agent-config.bak.yaml
kubectl apply -f agent-config.bak.yaml
kubectl -n hello-zhiduoxing rollout restart deployment/agent-api
```
