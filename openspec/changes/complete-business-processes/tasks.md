...
- [ ] 4.1 迁移 `users` 表：`password_hash` 必填、补充 `is_active` 索引；为种子用户初始化密码哈希
- [ ] 4.2 登录接口改为校验密码（passlib bcrypt），错误/缺失密码返回统一 401
- [ ] 4.3 `get_current_user` 校验用户 `is_active`，禁用用户旧令牌立即失效
- [ ] 4.4 新增 admin 用户管理 API：`GET /admin/users`、`POST /admin/users`、`PATCH /admin/users/{id}`（角色/部门/禁用），用户名重复返回 409
- [ ] 4.5 用户创建与角色调整写入审计日志（操作者、旧值、新值、时间）
- [ ] 4.6 补充密码校验、禁用、角色即时生效的单元测试

## 5. 外部系统接入与后台任务

- [ ] 5.1 `BaseTool` 增加 `_call_external` 抽象与 `provider` 配置开关（mock/http）
- [ ] 5.2 实现统一 HTTP 适配器：httpx 超时、指数退避重试、401/403 与 5xx 错误归一化为 `ToolResult`
- [ ] 5.3 CRM 工具接入适配器：查询客户、查询订单、创建任务；凭证从环境变量读取
- [ ] 5.4 邮件工具接入适配器：内部/外部邮件发送与补偿；Mock 保持为默认提供方
- [ ] 5.5 工单工具接入适配器：创建/更新工单
- [ ] 5.6 worker 实现三类异步任务：文档入库队列、Saga 补偿重试、审计本地缓存回写（Redis List + BLPOP，失败带重试计数与退避）
- [ ] 5.7 补充 HTTP 适配契约测试（测试桩服务）与 worker 任务处理测试
...
- [ ] 6.5 测试命名统一为 `test_should_<behaviour>_when_<condition>`，结构使用 Given-When-Then
- [ ] 6.6 在 `pyproject.toml` 增加覆盖率命令，CI/本地文档记录测试运行方式

## 7. 联调、文档与收尾

- [ ] 7.1 全量运行 pytest 与 W5-W9 eval 脚本，确认无回归
- [ ] 7.2 更新产品文档：业务流程补充知识库审核、审批超时、重规划、用户管理四段
- [ ] 7.3 更新使用案例手册与运维维护手册：新增运营审核、超时处理、用户管理、worker 启动说明
- [ ] 7.4 `openspec validate` 通过，等待归档
